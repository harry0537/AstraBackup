#!/usr/bin/env python3
"""
Project Astra NZ - Rover Manager V6
Simplified component management with auto-restart
"""
...
COMPONENTS = {
    195: {
        ...
        'script': 'combo_proximity_bridge_v6.py',
        'critical': True,
        'enabled': True
    },
    197: {
        'name': 'Data Relay',
        'script': 'rover_data_relay_v6.py',
        'critical': False,
        'enabled': True
    },
    198: {
        'name': 'Crop Monitor',
        'script': 'simple_crop_monitor_v6.py',
        'critical': False,
        'enabled': True
    }
}
...
class RoverManager:
    ...
    def check_hardware(self):
        ...
        checks = {
            'lidar': any(os.path.exists(p) for p in ['/dev/rplidar', '/dev/ttyUSB0']),
            'pixhawk': any(os.path.exists(p) for p in ['/dev/pixhawk', '/dev/ttyACM0']),
    ...
    def monitor(self):
        ...
        while self.running:
            ...
            try:
                with open('/tmp/proximity_v6.json', 'r') as f:
                    prox = json.load(f)
    ...
    def run(self):
        """Main execution"""
        print("=" * 60)
        print("PROJECT ASTRA NZ - ROVER MANAGER V6")
        print("=" * 60)
