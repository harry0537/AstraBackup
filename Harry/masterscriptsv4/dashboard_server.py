#!/usr/bin/env python3
"""
Project Astra NZ - AWS Dashboard Server v1
Real-time rover monitoring dashboard for Windows EC2
"""
...
class DashboardServer:
    def __init__(self):
        self.app = FastAPI(title="Project Astra NZ Dashboard", version="1.0")
        
        # Data storage
        self.rover_data_buffer = deque(maxlen=1000)  # Last 1000 data points
    ...
    def setup_routes(self):
        ...
        async def receive_rover_data(data: RoverDataPacket):
            ...
            try:
                ...
                logger.info(f"Received data from {data.rover_id} - "
                ...
                logger.error(f"Error processing rover data: {e}")
        ...
        @self.app.get("/api/rover/{rover_id}/latest")
        async def get_latest_rover_data(rover_id: str):
            """Get latest data for specific rover"""
            if rover_id in self.current_rover_status:
                return self.current_rover_status[rover_id]
            else:
        ...
        async def get_historical_data(hours: int = 1):
            ...
            historical = [
                data for data in self.rover_data_buffer
                if data.get('timestamp', 0) > cutoff_time
            ]
            return {
                'time_range_hours': hours,
                'data_points': len(historical),
                'data': historical[-100:]  # Last 100 points for performance
            }
...
<head>
    ...
    <title>Project Astra NZ - Rover Dashboard</title>
...
<body>
    <div class="header">
        <h1>🚜 Project Astra NZ - Rover Dashboard</h1>
    ...
    <div class="chart-container">
        ...
        <div class="telemetry-grid" id="telemetryGrid">
            <!-- Telemetry items will be populated dynamically -->
        </div>
    </div>
    ...
    <script>
        class DashboardApp {
            ...
            updateDataStats(data) {
                document.getElementById('packetsReceived').textContent = 'N/A';
            ...
            updateLidarDisplay(lidarData) {
                ...
                lidarData.data.forEach(point => {
                    const angle = (point.angle - 90) * Math.PI / 180; // Adjust for display
                    const distance = Math.min(point.distance, 2500); // Max 25m display
            ...
            updateTelemetry(telemetryData) {
                ...
                } else if (latest.type === 'system_status') {
                    items.push(
                        {label: 'Battery Voltage', value: latest.voltage_battery?.toFixed(2) || '--', unit: 'V'},
                        {label: 'Battery Current', value: latest.current_battery?.toFixed(1) || '--', unit: 'A'},
                        {label: 'Battery Remaining', value: latest.battery_remaining || '--', unit: '%'},
                        {label: 'CPU Load', value: latest.load?.toFixed(1) || '--', unit: '%'}
                    );
                }
...
def main():
    ...
    print("Project Astra NZ Dashboard Server v1")
    print("====================================")
    print("UGV-Server-Win2025 (i-03ae6c471d8c10b9c)")
    print("ZeroTier Network: UGV Network (41d49af6c276269e)")
    print(f"Dashboard URL: http://10.244.77.186:8080")
